...
public class FireAi extends Ai{
	private final static int MAX_WAIT_TIME_COUNT = 3; //瞄准目标3秒不能攻击切换行走
	private int waitTimeCount;			//瞄准时间次数
	
	public FireAi(AbstractInstance self) {
		super(self);
		...
		if(loopCount.isReachCycle()){
			...
			STC_AI_MOVE_MSG moveMsg = STC_AI_MOVE_MSG.newBuilder()
					...
					.setPosition(Coordinate3D.newBuilder()
							...
							.setPy(self.getY())
							.setPz(self.getZ()))
					.setTarget(target != null ? target.getId() : 0)
					.build();
