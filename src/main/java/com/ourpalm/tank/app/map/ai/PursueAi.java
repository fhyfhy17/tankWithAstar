...
public class PursueAi extends Ai{
	//不可行走BUFF效果ID
	...
	public PursueAi(AbstractInstance self){
		super(self);
	...
	public void update() {
		...
		if( target == null || target.isDeath() || !this.checkIsPursue(target)){
			LogCore.runtime.info("PursueAi update to move {} {} {}", target != null, !target.isDeath(), this.checkIsPursue(target));
		...
		if(Util.isEmpty(roads)){
			System.out.println("找不到路 追击 变 正常行走");
			//要把目标设置为空
			this.setTarget(null);
	...
	private void searchRoad(){
		...
		List<Node> beforeNodes = grid.getNearByNodes(target.getX(), target.getZ());
...
//			System.out.println("第一个坐标点 X="+targetNode.getPx() + "  Z="+targetNode.getPz());
		...
		//判断是否有目标点
		//计算一次循环所移动的距离
		float way = 2 * self.get(AttrType.n_speed) /10 * AIThreadHandler.AI_LOOP_TIME / 1000f;
