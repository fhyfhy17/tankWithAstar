...
public interface QuestApp extends Service{
	
	QuestTemplate getQuestTemplate(int questId);
	
	STC_QUEST_DRAW_MSG drawReward(int roleId, int questId);
	
	void login(int roleId, boolean first);

	void offline(int roleId);
	
	STC_QUEST_LIST_MSG getQuestList(int roleId);
	
	/** 领取活跃奖励 */
	STC_ACTIVE_DRAW_MSG drawActiveReward(int roleId, int type, int active);
