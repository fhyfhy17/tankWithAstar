...
@Command(
	...
	id = CORPS_MSG.CMD_ID.CTS_CORPS_SHOP_FLUSH_VALUE
)
public class CorpsShopFlushAction implements Action<Object>{
	...
	public MessageLite execute(ActionContext context, Object reqMsg) {
		...
		CorpsShopResult result = GameContext.getCorpsApp().handFlushShop(connect.getRoleId());
		
		if(logger.isDebugEnabled()){
			logger.debug("手动刷新商城: {}", JSON.toJSONString(result));
		}
