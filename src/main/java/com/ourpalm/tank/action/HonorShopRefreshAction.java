...
import com.ourpalm.tank.message.SHOP_MSG.STC_HONOR_SHOP_LIST_MSG;
...
@Command(
	...
	id = SHOP_MSG.CMD_ID.CTS_HONOR_SHOP_REFRESH_VALUE
)
public class HonorShopRefreshAction implements Action<Object> {
	...
	public MessageLite execute(ActionContext context, Object reqMsg) {
		...
		RoleShop roleShop = GameContext.getHonorShopApp().refreshShopList(connect.getRoleId());
		if(roleShop == null)
			return null;
		
		STC_HONOR_SHOP_LIST_MSG.Builder builder = STC_HONOR_SHOP_LIST_MSG.newBuilder();
		
		for(Integer itemId : roleShop.getGoodsIds()) {
			builder.addItemList(buildShopItem(itemId, roleShop.hadBuy(itemId)));
		}
		
		builder.setLeftTime(GameContext.getHonorShopApp().getLeftRefreshTime(connect.getRoleId()));
		builder.setRefreshCost(GameContext.getHonorShopApp().getRefreshCost(connect.getRoleId()));
		return builder.build();
	}
	
	private ShopItem buildShopItem(int itemId, boolean isHadBuy) {
		ShopItemTemplate template = GameContext.getHonorShopApp().getShopItemTemplate(itemId);
		if(template == null)
			return null;
